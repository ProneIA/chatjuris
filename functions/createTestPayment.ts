/**
 * Cria uma preferência de pagamento de teste no Mercado Pago
 * Body: { amount, title } — defaults: R$ 2,00
 */
...
Deno.serve(async (req) => {
  try {
    ...
    const amount = Number(body.amount) || 2.00;
    const title = body.title || `Teste de Pagamento - R$ ${amount.toFixed(2)}`;
    ...
    console.log('[createTestPayment] Preferência criada:', result.id, 'Valor:', amount);
